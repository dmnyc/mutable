...
interface AppState {
  ...
  addMutedItem: (
    item: MuteList[keyof MuteList][0],
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      setMuteListLastFetched: (timestamp) =>
        set({ muteListLastFetched: timestamp }),
      ...
      getNewItemsCount: (pack) => {
        ...
        (pack.list.pubkeys || []).forEach((p) => {
          if (!existingValues.has(p.value) && !importedItems.has(p.value))
            newCount++;
        });
        (pack.list.words || []).forEach((w) => {
          if (!existingValues.has(w.value) && !importedItems.has(w.value))
            newCount++;
        });
        (pack.list.tags || []).forEach((t) => {
          if (!existingValues.has(t.value) && !importedItems.has(t.value))
            newCount++;
        });
      ...
      markPackItemsAsImported: (packId, items) =>
        set((state) => {
      ...
      removeMutedItem: (value, category) =>
        set((state) => {
          const newList = { ...state.muteList };
          (newList[category] as any) = newList[category].filter(
            (item) => item.value !== value,
          );
          return { muteList: newList, hasUnsavedChanges: true };
        }),

      updateMutedItem: (oldValue, newValue, category, reason, eventRef) =>
        set((state) => {
          const newList = { ...state.muteList };
          const index = newList[category].findIndex(
      ...
      toggleItemPrivacy: (value, category) =>
        set((state) => {
          const newList = { ...state.muteList };
          const index = newList[category].findIndex(
            (item) => item.value === value,
      ...
      bulkSetPrivacy: (isPrivate) =>
        set((state) => {
          const newList = { ...state.muteList };
          newList.pubkeys = newList.pubkeys.map((item) => ({
            ...item,
            private: isPrivate,
          }));
          newList.words = newList.words.map((item) => ({
            ...item,
            private: isPrivate,
          }));
          newList.tags = newList.tags.map((item) => ({
            ...item,
            private: isPrivate,
          }));
          newList.threads = newList.threads.map((item) => ({
            ...item,
            private: isPrivate,
          }));
          return { muteList: newList, hasUnsavedChanges: true };
