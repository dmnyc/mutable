...
export default function PublicListCard({
  ...
  useEffect(() => {
    const loadPubkeyProfiles = async () => {
      if (!isExpanded || !list.list.pubkeys || list.list.pubkeys.length === 0)
        return;
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
                  ...
                  >
                    {importSuccess ? (
                      ...
                      <>
                        <Copy size={16} />
                        <span className="hidden sm:inline">
                          Add {newItemsCount > 0 && newItemsCount} to My Mute
                          List
                        </span>
            ...
            {list.description && (
              <div className="mb-3">
                <p
                  className={`text-sm text-gray-600 dark:text-gray-400 ${!descriptionExpanded ? "line-clamp-4" : ""}`}
                >
                  {list.description}
                </p>
          ...
          {session && newItemsCount > 0 && !allImported && (
            <div className="mt-3 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-sm text-blue-800 dark:text-blue-200">
                <strong>{newItemsCount}</strong> new{" "}
                {newItemsCount === 1 ? "item" : "items"} available to import
              </p>
            </div>
          )}
          ...
          {skippedBlacklisted > 0 && (
            <div className="mt-3 px-3 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
              <p className="text-sm text-amber-800 dark:text-amber-200">
                <strong>{skippedBlacklisted}</strong> blacklisted{" "}
                {skippedBlacklisted === 1 ? "profile" : "profiles"} skipped
                during import (removed via List Cleaner)
              </p>
            </div>
          )}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {loadingPubkeyProfiles
                      ? // Skeleton loaders
                        Array.from({
                          length: Math.min(3, list.list.pubkeys?.length || 0),
                        }).map((_, i) => (
                          <div
                            key={i}
                    ...
                    {(list.list.pubkeys?.length || 0) > ITEMS_PER_PAGE && (
                      <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          Showing {(pubkeyPage - 1) * ITEMS_PER_PAGE + 1}-
                          {Math.min(
                            pubkeyPage * ITEMS_PER_PAGE,
                            list.list.pubkeys?.length || 0,
                          )}{" "}
                          of {list.list.pubkeys?.length || 0}
                        </p>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() =>
                              setPubkeyPage((p) => Math.max(1, p - 1))
                            }
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {(list.list.words?.length || 0) > ITEMS_PER_PAGE && (
                      <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          Showing {(wordPage - 1) * ITEMS_PER_PAGE + 1}-
                          {Math.min(
                            wordPage * ITEMS_PER_PAGE,
                            list.list.words?.length || 0,
                          )}{" "}
                          of {list.list.words?.length || 0}
                        </p>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() =>
                              setWordPage((p) => Math.max(1, p - 1))
                            }
                            disabled={wordPage === 1}
                            className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                            title="Previous page"
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    {(list.list.tags?.length || 0) > ITEMS_PER_PAGE && (
                      <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          Showing {(tagPage - 1) * ITEMS_PER_PAGE + 1}-
                          {Math.min(
                            tagPage * ITEMS_PER_PAGE,
                            list.list.tags?.length || 0,
                          )}{" "}
                          of {list.list.tags?.length || 0}
                        </p>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() =>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full p-6">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              ...
              action cannot be undone. A deletion event will be published to the
              network.
            </p>
