...
export default function PublicLists() {
  ...
  const {
    publicLists,
    setPublicLists,
    setPublicListsLoading,
    publicListsLoading,
  } = useStore();

  const [viewMode, setViewMode] = useState<"browse" | "my-packs">("my-packs");
  const [searchType, setSearchType] = useState<"author" | "name" | "browse">(
    "browse",
  );
  const [searchQuery, setSearchQuery] = useState("");
  const [searchError, setSearchError] = useState<string | null>(null);
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [editingPack, setEditingPack] = useState<PublicMuteList | undefined>(
    undefined,
  );
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchType === "author") {
        ...
        // If it's already an npub or nprofile, convert to hex
        if (
          authorPubkey.startsWith("npub") ||
          authorPubkey.startsWith("nprofile")
        ) {
        ...
        // If it's not a hex pubkey, try to search for profiles by username/NIP-05
          ...
          if (profiles.length === 0) {
            setSearchError(
              `No user found with username or NIP-05: "${authorPubkey}"`,
            );
          ...
          const displayName =
            profiles[0].display_name || profiles[0].name || profiles[0].nip05;
          if (profiles.length > 1) {
            setSearchError(
              `Found ${profiles.length} users matching "${searchQuery}". Showing packs from: ${displayName}`,
            );
          } else {
        ...
        events = await searchPublicListsByName(
          searchQuery.trim(),
          session.relays,
        );
      } else {
        ...
        events = await fetchAllPublicPacks(
          session.relays,
          100,
          category,
          false,
      ...
      setSearchError(
        error instanceof Error
          ? error.message
          : "Failed to search community packs",
      );
    } finally {
      setPublicListsLoading(false);
    }
  ...
  const handleSelectProfile = async (profile: Profile) => {
    ...
    try {
      ...
      const events = await searchPublicListsByAuthor(
        profile.pubkey,
        session!.relays,
      );
      const parsedLists = await Promise.all(events.map(parsePublicListEvent));
          ...
          {!loadingUserPacks && userPacks.length > 0 && (
            <div className="space-y-4">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                You have {userPacks.length}{" "}
                {userPacks.length === 1 ? "pack" : "packs"}
              </p>
              {userPacks.map((list) => (
                <PublicListCard
                  key={list.id}
      ...
      {viewMode === "browse" && (
        <>
          ...
          {!publicListsLoading && publicLists.length > 0 && (
            <div className="space-y-4">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Found {publicLists.length}{" "}
                {publicLists.length === 1 ? "pack" : "packs"}
              </p>
          ...
          {!publicListsLoading && publicLists.length === 0 && !searchError && (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-2">
                {searchType === "browse"
                  ? 'Click "Load Community Packs" to get started'
                  : "Search for community packs"}
              </p>
