...
// Thread event info cache
interface ThreadEventInfo {
  event: Event;
...
export default function MuteListCategory({
  ...
  const { addMutedItem, removeMutedItem, updateMutedItem, toggleItemPrivacy } =
    useStore();
  ...
  const handleAdd = () => {
    ...
    try {
      ...
      // Convert nevent/note to hex for threads
      if (
        category === "threads" &&
        (finalValue.startsWith("nevent") || finalValue.startsWith("note"))
      ) {
  ...
  const safeHexToNote = (eventId: string): string => {
    try {
      return hexToNote(eventId);
    } catch {
      // If conversion fails, return truncated hex
      return eventId.slice(0, 16) + "..." + eventId.slice(-8);
    }
  };
  ...
  // Load thread events and author profiles for threads (only for current page)
  useEffect(() => {
    const loadThreadEvents = async () => {
      if (category !== "threads" || !session) return;

      setLoadingThreads(true);
      const eventsMap = new Map<string, ThreadEventInfo>(threadEvents);
      ...
      const fetchPromises = currentPageItems.map(async (item) => {
        // Skip if already loaded
        if (eventsMap.has(item.value)) return;

        try {
          const event = await fetchEventById(item.value, session.relays);
          if (event) {
            // Fetch author profile
            const authorProfile = await fetchProfile(
              event.pubkey,
              session.relays,
            );
            eventsMap.set(item.value, {
              event,
          ...
          console.error(
            `Failed to fetch thread event for ${item.value}:`,
            error,
          );
        }
      });
        ...
        <div className="p-4 sm:p-6 w-full max-w-full">
          ...
          <div className="space-y-2 mb-4">
            {items.length === 0 ? (
              ...
              (loadingThreads && category === "threads") ? (
              // Show skeletons while loading profiles or threads
              currentItems.map((item, idx) => <SkeletonLoader key={idx} />)
            ) : (
              currentItems.map((item) => {
                const profile =
                  category === "pubkeys" ? profiles.get(item.value) : null;
                const displayName = profile?.display_name || profile?.name;
                const threadInfo =
                  category === "threads" ? threadEvents.get(item.value) : null;
                const threadAuthor = threadInfo?.authorProfile;
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full gap-2">
                      ...
                      >
                        ...
                        {/* Avatar/icon for threads */}
                        {category === "threads" && (
                          <>
                            {threadAuthor?.picture ? (
                              // eslint-disable-next-line @next/next/no-img-element
                              <img
                                src={threadAuthor.picture}
                                alt={
                      ...
                      <div className="flex space-x-1 sm:space-x-2 flex-shrink-0 justify-end w-full sm:w-auto">
                        ...
                        <button
                          ...
                          title={
                            item.private
                              ? "Private (encrypted) - Click to make public"
                              : "Public (visible to all) - Click to make private"
                          }
                        >
                          {item.private ? (
                            <Lock size={16} />
